...
bool CheckAndLogError(BOOL success, NSError* error) {
  if (!success) {
    ...
    std::cout << StdStringFromNSString(msg);
    return false;
  }
  return true;
}
...
std::string GetUniqueStrByUUID() {
    CFUUIDRef uuidObj = CFUUIDCreate(nil);
    NSString *uuidString = (__bridge_transfer NSString*)CFUUIDCreateString(nil, uuidObj);
    CFRelease(uuidObj);
    return StdStringFromNSString(uuidString);
}
...
std::string GetDeviceName() {
  ...
  if (!raw_name.compare("x86_64"))
    return std::string("Simulator");
  std::cout << "Failed to find device name (" << raw_name << ")";
  return raw_name;
}

std::string GetTimeInterval(UInt64 *val) {
    UInt64 recordTime = [[NSDate date] timeIntervalSince1970] * 1000000;
    if (val)
        *val = recordTime;
    NSString *timeS = [NSString stringWithFormat:@"%.16llu", recordTime];
    return StdStringFromNSString(timeS);
}
